...
import { ChevronUp, ChevronDown, ChevronsUpDown, Plus, Search, Eye, Edit, Trash2, Upload, RotateCcw, Trash } from 'lucide-react';
...
interface AmenitiesManagementProps {
    amenities: Amenity[];
    trashedAmenities: Amenity[];
}

export default function AmenitiesManagement({ amenities, trashedAmenities }: AmenitiesManagementProps) {
    ...
    const { post: restoreAmenity, processing: restoreProcessing } = useForm();

    const handleCreateSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        postCreate('/admin/amenities', {
            forceFormData: true,
            onSuccess: () => {
                resetCreate();
    ...
    const handleEditSubmit = (e: React.FormEvent) => {
        ...
        postEdit(`/admin/amenities/${editingAmenity.amenity_id}`, {
            forceFormData: true,
            onSuccess: () => {
                resetEdit();
    ...
    const handleEdit = (amenity: Amenity) => {
        ...
        setEditData({
            ...
            price_per_use: amenity.price_per_use.toString(),
            image: null,
            _method: 'PUT',
        });
        setImagePreview(null);
    ...
    const handleRestoreClick = (amenity: Amenity) => {
        setAmenityToDelete(amenity);
    ...
    const handleRestoreConfirm = () => {
        ...
        restoreAmenity(`/admin/amenities/${amenityToDelete.amenity_id}/restore`, {
            onSuccess: () => {
    ...
    const getImageUrl = (imagePath: string | null) => {
        if (!imagePath) return 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=400&h=300&fit=crop';
    ...
    const displayedAmenities = showTrashed ? trashedAmenities : amenities;

    const columns = useMemo<ColumnDef<Amenity>[]>(
        () => [
            {
                accessorKey: 'amenity_id',
                header: 'ID',
            ...
            },
            {
                ...
                cell: ({ row }: any) => (
                    <div className="flex items-center">
                        <img
                            src={getImageUrl(row.original.image_path)}
                            alt={row.original.amenity_name}
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                    ...
                    <div className="max-w-full px-8 py-6">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                {/* Toggle Trashed Button */}
                                <Button
                                    onClick={() => setShowTrashed(!showTrashed)}
                                    variant={showTrashed ? "default" : "outline"}
                                    className={showTrashed ? "bg-gray-600 hover:bg-gray-700" : ""}
                                >
                                    <Trash className="mr-2 h-4 w-4" />
                                    {showTrashed ? `Active (${amenities.length})` : `Trash (${trashedAmenities.length})`}
                                </Button>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {globalFilter && (
                                    <p className="text-sm text-gray-600 whitespace-nowrap">
                                        Found {table.getFilteredRowModel().rows.length} of {displayedAmenities.length}
                                    </p>
                                )}
                            </div>
                        </div>
            ...
            <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Are you sure you want to move "{amenityToDelete?.amenity_name}" to trash? You can restore it later.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex justify-end gap-2 pt-4">
                        ...
                        <Button
                            ...
                            disabled={deleteProcessing}
                        >
                            {deleteProcessing ? 'Moving...' : 'Move to Trash'}
                        </Button>
                    </div>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isRestoreDialogOpen} onOpenChange={setIsRestoreDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Restore Amenity</DialogTitle>
                        <DialogDescription>
                            Are you sure you want to restore "{amenityToDelete?.amenity_name}"?
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex justify-end gap-2 pt-4">
                        <Button
